...
replaceBtn.addEventListener("click", async () => {
    ...
    chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: replaceFunc,
        args: [input.value, quotes[randIndex]],
    });
    input.value = '';      
});
...
kanyeBtn.addEventListener("click", async () => {
    let [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: yeFunc,
        args: [input.value, quotes],
    });
    input.value = '';      
});
...
function audioFunc(){
    let randIndex = Math.floor(Math.random() * 9) + 3;
    let kanyeClip = new Audio(chrome.runtime.getURL(`audio/kanyeClip${randIndex}.wav`));
    const body = document.querySelector('body');
    body.addEventListener("click", ()=>{
        kanyeClip.pause();
        randIndex === 11 ? randIndex = 3 : randIndex++;
        kanyeClip = new Audio(chrome.runtime.getURL(`audio/kanyeClip${randIndex}.wav`));
        kanyeClip.play();
    }, true);
}


function yeFunc(findVal, quotes){
    // if(findVal[0] === '/'){
    //     for(let i=findVal.length -1; i >= 0; i--){
    //         if(findVal[i] === '/'){
    //             findVal = new RegExp(findVal.slice(1,i), findVal.slice(i + 1))
    //             break;
    //         }
    //     }
    // } else {
    //     findVal = new RegExp(findVal, 'ig');
    // }
    //recursive function takes an element as a param
    const replace = (element, queue = element.childNodes)=>{
        // replace inner text with input.value (testing: 'kanye was here')

        // set queue as an array of all child elements
        for(const node of queue){
            // Pick random index for kanye picture
            const randIndex = Math.floor(Math.random() * 10);
            const randIndexQuotes = Math.floor(Math.random() * quotes.length);
            //const randQuote = Math.floor(Math.random() * quotes.length)
            const kanyeImg = chrome.runtime.getURL(`images/kanye${randIndex}.png`);
            if(node.nodeType === Node.TEXT_NODE && node.nodeValue.trim() !== ""){
                node.textContent = node.textContent.replace(node.textContent, (quotes[randIndexQuotes] + ' '));
                // node.textContent = node.textContent.replace(findVal, quotes[randQuote]);
            }
            else if(node.nodeType === Node.ELEMENT_NODE){
                if(node.hasAttribute('value')){
                    node.setAttribute('value',
                        // node.getAttribute('value').replace(findVal, quotes[randIndexQuotes])
                        node.getAttribute('value').replace(findVal, (quotes[randIndexQuotes] + ' '))
                    );
                }
                if(node.nodeName === 'IMG'){
                    node.setAttribute('src', kanyeImg);
                }
                replace(node);
            }
        }
...
function replaceFunc(findVal, quote){
    if(findVal[0] === '/'){
        for(let i=findVal.length -1; i >= 0; i--){
            if(findVal[i] === '/'){
                findVal = new RegExp(findVal.slice(1,i), findVal.slice(i + 1))
    ...
    const replace = (element, queue = element.childNodes)=>{
        ...
        for(const node of queue){
            if(node.nodeType === Node.TEXT_NODE && node.nodeValue.trim() !== ""){
                node.textContent = node.textContent.replace(findVal, (quote + ' '));
            }
            else if(node.nodeType === Node.ELEMENT_NODE){
                if(node.hasAttribute('value')){
                    node.setAttribute('value',
                        node.getAttribute('value').replace(findVal, (quote + ' '))
                    );
                }
                replace(node);
            }
...
function imageFunc(findVal, quotes){
...
const quotes = [
    ...
    "Shut the *bleep* up I will *bleep*ing laser you with alien *bleep*ing eyes and explode your *bleep*ing head",
